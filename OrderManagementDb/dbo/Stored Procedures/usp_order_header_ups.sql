...
ELSE BEGIN
	BEGIN TRY
	SELECT @order_ship_date = COALESCE(@order_ship_date, (SELECT MIN(order_item_ship_date) FROM order_item WHERE order_key = @order_key));
	...
	WHERE [order_key] = @order_key
		...
		OR @order_ship_date IS NOT NULL
