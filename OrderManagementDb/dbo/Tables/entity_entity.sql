CREATE TABLE [dbo].[entity_entity] (
    ...
    [primary_entity_role_key]   INT           NOT NULL,
    [secondary_entity_key]      INT           NOT NULL,
    [secondary_entity_type_key] INT           NOT NULL,
    [secondary_entity_role_key] INT           NOT NULL,
    ...
    CONSTRAINT [FK_primary_entity_role_key_entity_role_key] FOREIGN KEY ([primary_entity_role_key]) REFERENCES [dbo].[entity_role] ([entity_role_key]),
    ...
    CONSTRAINT [FK_secondary_entity_role_key_entity_role_key] FOREIGN KEY ([secondary_entity_role_key]) REFERENCES [dbo].[entity_role] ([entity_role_key])
);


GO
CREATE UNIQUE CLUSTERED INDEX [NonClusteredIndex-20150701-155340]
    ON [dbo].[entity_entity]([primary_entity_key] ASC, [primary_entity_type_key] ASC, [primary_entity_role_key] ASC, [secondary_entity_key] ASC, [secondary_entity_type_key] ASC, [secondary_entity_role_key] ASC, [entity_entity_seq] ASC);


GO

CREATE TRIGGER [dbo].[tgr_entity_entity_audit]
    ON [dbo].[entity_entity]
    FOR INSERT, UPDATE
    AS
    BEGIN
        SET NOCOUNT ON
...
CREATE TRIGGER [dbo].[tgr_entity_entity_audit_del]
    ON [dbo].[entity_entity]
    AFTER DELETE
    AS
    BEGIN
